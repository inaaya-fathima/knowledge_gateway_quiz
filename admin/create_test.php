            ...
            if (json_last_error() === JSON_ERROR_NONE && is_array($data)) {
                ...
                foreach ($data as $q) {
                    if (
                        ...
                        if (
                            $answer_index === false && is_numeric($q['answer'])
...
<body class="bg-gradient">
    <div class="page-wrap">
        ...
        <div class="header-bar">
            <div>
                ...
                <p style="margin:0; font-size:0.82em; color:var(--text-faint);">Add practice questions or set up an
                    official test room</p>
            </div>
        ...
        <div class="method-tabs">
            <button class="method-tab" id="tab-form" onclick="switchTab('form')">📝 Manual MCQ<br><small
                    style="opacity:.6;">Add one question</small></button>
            <button class="method-tab" id="tab-json" onclick="switchTab('json')">📦 JSON Batch<br><small
                    style="opacity:.6;">Import many at once</small></button>
            <button class="method-tab" id="tab-testroom" onclick="switchTab('testroom')">🎯 Test Room<br><small
                    style="opacity:.6;">Generate test code</small></button>
        </div>
    ...
    ══════════════════════════════════════ -->
        <div class="method-panel glass-card" id="panel-form">
            <h3 style="margin-bottom:4px; font-family:'Inter',sans-serif; font-size:1.05em;">Add an Official Test
                Question</h3>
            <p style="color:var(--text-muted); font-size:0.85em; margin-bottom:20px;">
                These questions are <strong style="color:var(--red-bright)">exclusive to official tests</strong> — they
                will <em>not</em> appear in the student practice zone.
            </p>
            ...
            <form action="create_test.php" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="topic" required placeholder="e.g. Python, PHP, Java, Class Test"
                        value="<?php echo htmlspecialchars($_POST['topic'] ?? ''); ?>">
                </div>

                <div class="form-group">
                    <label>Question Text <span style="color:var(--red-bright)">*</span></label>
                    <input type="text" name="question" required placeholder="e.g. What does HTML stand for?"
                ...
                <div class="form-group">
                    ...
                    <select name="correct_answer" required>
                        ...
                        <option value="0" <?php echo (($_POST['correct_answer'] ?? '') === '0') ? 'selected' : ''; ?>>A —
                            Option A</option>
                        <option value="1" <?php echo (($_POST['correct_answer'] ?? '') === '1') ? 'selected' : ''; ?>>B —
                            Option B</option>
                        <option value="2" <?php echo (($_POST['correct_answer'] ?? '') === '2') ? 'selected' : ''; ?>>C —
                            Option C</option>
                        <option value="3" <?php echo (($_POST['correct_answer'] ?? '') === '3') ? 'selected' : ''; ?>>D —
                            Option D</option>
                    </select>
                </div>
    ...
    ══════════════════════════════════════ -->
        <div class="method-panel glass-card" id="panel-json">
            <h3 style="margin-bottom:4px; font-family:'Inter',sans-serif; font-size:1.05em;">JSON Batch — Official Test
                Questions</h3>
            <p style="color:var(--text-muted); font-size:0.85em; margin-bottom:14px;">
                Bulk-import questions into the <strong style="color:var(--red-bright)">official test pool</strong>.
                These are hidden from practice mode.
            </p>
    ...
    ══════════════════════════════════════ -->
        <div class="method-panel glass-card" id="panel-testroom">
            <h3 style="margin-bottom:4px; font-family:'Inter',sans-serif; font-size:1.05em;">Generate Official Test Room
            </h3>
            <p style="color:var(--text-muted); font-size:0.85em; margin-bottom:20px;">
                Create a test session from the <strong style="color:var(--red-bright)">official test question
                    pool</strong>. Students use the generated code to enter.
            </p>

            <?php if (empty($topics)): ?>
                <div style="text-align:center; padding:24px; color:var(--text-muted);">
                    <p>No official test topics yet.<br>
                        Use the <strong>Manual MCQ</strong> or <strong>JSON Batch</strong> tabs to add official test
                        questions first.</p>
                ...
                <form action="create_test.php" method="POST">
                        ...
                        <select name="topic" id="topicSelect" required onchange="updateAvail(this)">
                            ...
                            <?php foreach ($topics as $t): ?>
                                <option value="<?php echo htmlspecialchars($t['topic']); ?>"
                                    data-count="<?php echo $t['cnt']; ?>" <?php echo (($_POST['topic'] ?? '') === $t['topic']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="num_questions" id="numQInput"
                            value="<?php echo htmlspecialchars($_POST['num_questions'] ?? '10'); ?>" min="1" max="100"
                            required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="time_limit"
                            value="<?php echo htmlspecialchars($_POST['time_limit'] ?? '15'); ?>" min="0" max="300"
                            required>
                        <div class="help-text">Set to <strong>0</strong> for no time limit.</div>
                    </div>

                    <div
                        style="background:var(--bg); border:1px solid var(--border); border-radius:var(--radius-sm); padding:14px 16px; margin-bottom:18px; font-size:0.85em; color:var(--text-muted);">
